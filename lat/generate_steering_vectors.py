...
def generate_and_save_steering_vectors(
    ...
    for p_tokens, n_tokens in tqdm(dataset, desc="Processing prompts"):
        ...
        for layer in layers:
            ...
            p_activations_value = p_activations_value[0, token_idx, :].detach().cpu()
            p_activations_query = p_activations_query[0, token_idx, :].detach().cpu()
            positive_activations[layer].append((p_activations, p_activations_value, p_activations_query))
        model.reset_all()
        ...
        for layer in layers:
            ...
            n_activations_value = n_activations_value[0, token_idx, :].detach().cpu()
            n_activations_query = n_activations_query[0, token_idx, :].detach().cpu()
            negative_activations[layer].append((n_activations, n_activations_value, n_activations_query))
    for layer in layers:
        positive_activations_layer = [x[0] for x in positive_activations[layer]]
        positive = torch.stack(positive_activations_layer)
        negative_activations_layer = [x[0] for x in negative_activations[layer]]
        negative = torch.stack(negative_activations_layer)
        vec = (positive - negative).mean(dim=0)
        positive_activations_value_layer = [x[1] for x in positive_activations[layer]]
        positive_value = torch.stack(positive_activations_value_layer)
        negative_activations_value_layer = [x[1] for x in negative_activations[layer]]
        negative_value = torch.stack(negative_activations_value_layer)
        vec_value = (positive_value - negative_value).mean(dim=0)
        positive_activations_query_layer = [x[2] for x in positive_activations[layer]]
        positive_query = torch.stack(positive_activations_query_layer)
        negative_activations_query_layer = [x[2] for x in negative_activations[layer]]
        ...
        for activation_type, vectors in zip(["" , "_value", "_query"], 
                                            [(vec, positive, negative), 
                                             (vec_value, positive_value, negative_value), 
                                             (vec_query, positive_query, negative_query)]):
            vec_, positive_, negative_ = vectors
            torch.save(vec_, f"{output_folder}/vec{activation_type}_layer_{layer}.pt")
            torch.save(positive_, f"{output_folder}/positive{activation_type}_layer_{layer}.pt")
            torch.save(
                negative_,
