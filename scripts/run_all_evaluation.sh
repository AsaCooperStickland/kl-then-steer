...
# for m in ${base_path}/run2_working_concepts_${f}/checkpoint-12000 ; do for e in refusal ; do sbatch submit_eval.sh $m $e /scratch/alc9734/latent-adversarial-training/datasets/ big ; done; done ;
...
for m in /vast/work/public/ml-datasets/llama-2/Llama-2-13b-chat-hf ; do for e in refusal ; do sbatch submit_eval.sh $m $e big ; done; done ;
for m in /vast/work/public/ml-datasets/llama-2/Llama-2-7b-chat-hf ; do for e in refusal ; do sbatch submit_eval.sh $m $e big ; done; done ;
for m in ${base_path}/run2_persuasion_0.5 ; do for e in refusal ; do sbatch submit_eval.sh $m $e big ; done; done ;
for m in ${base_path}/run2_no_steer ; do for e in refusal ; do sbatch submit_eval.sh $m $e big ; done; done ;