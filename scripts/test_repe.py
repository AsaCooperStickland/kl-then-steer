...
def generate_with_vector(trainer, tokenizer, questions, directory, custom_args, question_type="", temperature=0.0):
    ...
    for multiplier in [-3.0, -1.5, 0.0, 1.5, 3.0]:
        answers = []
        activations = trainer.steering.get_shift(coeff=multiplier, layer_id=layer_ids, num_pairs=10)
    ...
    # Save results
    with open(f"{custom_args['model_name_or_path']}/{directory}/{question_type}results.json", "w") as jfile:
...
def main():
    ...
    os.makedirs(f"{custom_args['model_name_or_path']}/{output_folder_name}", exist_ok=True)
